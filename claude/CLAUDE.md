...
A PreToolUse hook (`remove-redundant-git-c.sh`) enforces this by automatically stripping `-C <path>` from any git command where the path matches the current working directory. Using `git -C` to target a genuinely different directory is unaffected.
